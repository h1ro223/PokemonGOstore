...
let selectedIndex = null; // 現在選択中
let modalType = null; // "detail" or "form"
let editingIdx = null; // 編集時のindex
...
function openFormModal(idx = null) {
  ...
  ocrImage.onchange = async function () {
    ...
    ocrStatus.textContent = "内容反映中…";
    // ボックス名
    const nameMatch = text.match(/([^\s]{2,30})(ボックス|BOX)/i);
    if (nameMatch)
      document.querySelector('input[name="name"]').value = nameMatch[0].trim();
    // 値段
    const priceMatch = text.match(/(\d{2,6})\s*コイン/);
    if (priceMatch)
      document.querySelector('input[name="poke_coins"]').value = priceMatch[1];
    // アイテムと個数
    const items = [];
    const itemRegex =
      /([\u30A0-\u30FF\u3040-\u309F\u4E00-\u9FFFＡ-Ｚa-zA-Z0-9\.\s]+?)\s*[×x](\d{1,3})/g;
    let match;
    while ((match = itemRegex.exec(text)) !== null) {
      items.push({
        name: match[1].replace(/x\d+$/, "").trim(),
    ...
    setTimeout(() => (ocrStatus.textContent = ""), 2100);
    ocrImage.value = "";
  };
