...
function renderBoxList() {
  ...
  if (boxList.length === 0) {
    boxListDiv.innerHTML =
      '<div style="color:#aaa;font-size:1em;padding:1em 0.2em;">まだ記録はありません</div>';
    return;
  }
...
function openDetailModal(idx) {
        ...
        <div class="items-grid">
          ${box.items
            ...
            <div class="item-detail-card">
              ${
                it.image_url
                  ? `<img src="${it.image_url}" alt="item">`
                  : `<img src="https://raw.githubusercontent.com/h1ro223/pokebox-assets/main/ball.png" style="opacity:0.3;">`
              }
      ...
      <div style="margin-top:0.6em; font-size:1em;">
        ...
        <b>セール：</b>${
          box.sale && box.sale.is_sale
            ? `はい <span style="color:#f72585;">(${
                box.sale.percent_off || "-"
              }%OFF/元値: ${box.sale.original_price || "-"})</span>`
            : "いいえ"
        }<br>
  ...
  document.getElementById("modal-edit-btn").onclick = () => {
    closeModal();
    setTimeout(() => openFormModal(idx), 240);
  };
...
function openFormModal(idx = null) {
  ...
  let formHtml = `
    <form id="box-form" autocomplete="off" class="modal-card">
      <div class="ocr-row">
        ...
        <button type="button" id="ocr-btn">画像から自動入力</button>
        <span id="ocr-status" style="margin-left:1em;color:#7c3aed;font-size:0.97em;"></span>
      </div>
      <h2 style="margin-bottom:0.85em;">${
        isEdit ? "ボックス編集" : "ボックス追加"
      }</h2>
      <div class="form-row">
        <label>★ボックス名
  ...
  ocrBtn.onclick = () => ocrImage.click();
  ocrImage.onchange = async function () {
    if (!ocrImage.files[0]) return;
    ocrStatus.textContent = "OCR解析中...";
    const { data } = await Tesseract.recognize(ocrImage.files[0], "jpn", {
      logger: (m) => {},
    });
    let text = data.text;
    text = text
    ...
    let items = [];
    const itemRegex =
      /([ァ-ンヴーa-zA-Z一-龠々〆〇０-９0-9.\s]+?)×\s*([0-9０-９]+)/g;
    let match;
    while ((match = itemRegex.exec(text)) !== null) {
      let name = match[1].replace(/\s+/g, "").trim();
      let count = Number(
        match[2].replace(/[０-９]/g, (s) =>
          String.fromCharCode(s.charCodeAt(0) - 0xfee0)
        )
      );
      if (name && count) items.push({ name, count, image_url: "" });
    }
    if (items.length === 0) {
      const lineRegex =
        /([ァ-ンヴーa-zA-Z一-龠々0-9\s]+)\s*[x×✕✖＊*]\s*([0-9]+)/g;
    ...
    if (items.length === 0) {
      const loose = /([ァ-ンヴーa-zA-Z一-龠々0-9]+)(?:x|×)([0-9]{1,3})/g;
    ...
    itemListDiv.innerHTML = "";
    if (items.length) {
      for (const it of items) addItemInput(it);
      ocrStatus.textContent = "自動入力しました！";
    } else {
      ocrStatus.textContent = "認識できませんでした…";
    }
    setTimeout(() => (ocrStatus.textContent = ""), 2400);
    let nameMatch = text.match(/([^\s]{2,30})(ボックス|BOX)/i);
    if (nameMatch)
      document.querySelector('input[name="name"]').value = nameMatch[0].trim();
    let priceMatch = text.match(/([0-9０-９]{2,6})\s*コイン/);
    if (priceMatch)
      document.querySelector('input[name="poke_coins"]').value =
        priceMatch[1].replace(/[０-９]/g, (s) =>
          String.fromCharCode(s.charCodeAt(0) - 0xfee0)
        );
    ocrImage.value = "";
  };
